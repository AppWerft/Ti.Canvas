...
#define TAG "ti.canvas"

using namespace v8;

static Persistent<Object> bindingCache;

static void Canvas_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Canvas.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::CanvasBindings::lookupGeneratedInit(
...
static void Canvas_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Canvas_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Canvas_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::CanvasBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry CanvasBinding = {
	"ti.canvas",
	Canvas_init,
	Canvas_dispose
};
...
Java_ti_canvas_CanvasBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.canvas", &CanvasBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::CanvasBindings::lookupGeneratedInit));
}
